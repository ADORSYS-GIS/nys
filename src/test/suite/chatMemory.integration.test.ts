...
describe("Chat Memory Integration", () => {
  it("should persist and reload chat history", async () => {
    ...
    expect(vars.chat_history.length).to.be.gte(2);
    expect(vars.chat_history[0].inputValues.input).to.equal("Hello");
    expect(vars.chat_history[1].outputValues.output).to.equal("I'm fine.");
  });
  ...
  it("should clear chat history", async () => {
    ...
    expect(vars.chat_history.length).to.equal(0);
  });
});